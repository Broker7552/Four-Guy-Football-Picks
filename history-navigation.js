(()=>{
  ...
  const W1=[['Ross','9–7','9 pts','−$4'],['Scott','8–8','8 pts','−$10'],['Jim','8–8','8 pts','−$10'],['Ken','10–6','10 pts','+$24']];
  ...
  async function renderHistory(n){
    ...
    try{
      ...
      const finalStats={Ross:['15–7','16 pts','+$24'],Scott:['8–14','10 pts','−$6'],Jim:['10–12','10 pts','−$6'],Ken:['8–14','9 pts','−$12']};
      const liveGames=d.live?.games||[];const games=d.g.map(x=>{const lg=liveGames.find(z=>z.id===x.id)||{};const winner=lg.ats_winner||null;const final=(lg.away_score!==null&&lg.away_score!==undefined&&lg.home_score!==null&&lg.home_score!==undefined)?'<div class="muted">Final: '+esc(x.away_team)+' '+lg.away_score+'–'+lg.home_score+' '+esc(x.home_team)+' · ATS: '+esc(winner||'—')+'</div>':'';return '<div class="fg-hist-game"><div class="fg-hist-match"><b>'+esc(x.away_team)+' @ '+esc(x.home_team)+'</b><span>'+esc(x.favorite_team)+' '+esc(x.spread)+'</span></div>'+final+'<div class="fg-hist-picks">'+players.map(n=>{const v=d.p[x.id]?.[n]||'—';const push=winner==='Push',ok=winner&&v===winner;return '<div class="'+(winner?(push?'':ok?'fg-pick-win':'fg-pick-loss'):'')+'"><b>'+n+'</b><br><span>'+esc(v)+(winner&&!push?' '+(ok?'✓':'✕'):'')+'</span></div>';}).join('')+'</div></div>';}).join('');
      card.innerHTML='<h2>Week 2 — Historical Record</h2><p class="muted">Final Week 2 standings, money and picks.</p><div class="fg-standings">'+players.map((n,i)=>'<div class="fg-standing"><small>'+(i+1)+'</small><b>'+n+'</b><div>'+finalStats[n][0]+' · '+finalStats[n][1]+' · <span class="'+(finalStats[n][2].startsWith('+')?'fg-positive':'fg-negative')+'">'+finalStats[n][2]+'</span></div></div>').join('')+'</div><h3 style="margin-top:28px">Week 2 Games</h3><div class="fg-hist-cards">'+games+'</div>';
  ...
  function rebuildNav(){
    ...
    let glance=buttons.find(b=>['week at a glance','dashboard'].includes(b.textContent.trim().toLowerCase()));
    if(!glance){glance=document.createElement('button');glance.type='button';}
    glance.textContent='Week at a Glance';glance.onclick=()=>show('home');

    let roster=buttons.find(b=>['current picks','week 3','current games','weekly picks','games roster'].includes(b.textContent.trim().toLowerCase()));
    if(!roster){roster=document.createElement('button');roster.type='button';}
    roster.textContent='Games Roster';roster.onclick=()=>show('picks');
    ...
    const admin=buttons.find(b=>b.id==='adminBtn');
    nav.innerHTML='';
    [glance,roster,live,stand,hist,rules,admin].filter(Boolean).forEach(b=>nav.appendChild(b));
  }
  function selector(){
    const card=document.querySelector('#history>.card');if(!card)return;
