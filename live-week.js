  ...
  function projection(data){
    ...
    const scenarios=scenarioGames.map(g=>{
      const sides=[g.away_team,g.home_team].map(team=>{
        ...
        const totals=ORDER.map(n=>'<span><b>'+esc(n)+'</b> '+(nowPts[n]+(helped.includes(n)?w:0))+' pts</span>').join('');
