...
(() => {
  ...
  async function initialize(){
    ...
    el('test-week').replaceChildren();for(const w of weeks){const o=document.createElement('option');o.value=w.id;o.textContent=w.season+' · Week '+w.week_number;el('test-week').append(o);}const current=weeks.slice().sort((a,b)=>b.week_number-a.week_number)[0];if(current)el('test-week').value=current.id;
