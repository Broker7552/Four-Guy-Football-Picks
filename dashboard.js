(()=>{
  ...
  async function loadDashboard(){
    ...
    try{
      ...
      document.getElementById('dash-grid').innerHTML='<div class="dash-grid">'+ORDER.map(name=>{const s=(cur.standings||[]).find(x=>x.name===name)||{wins:0,losses:0,pushes:0,points:0};const rec=s.wins+'-'+s.losses+(s.pushes?'-'+s.pushes+' P':'');return '<div class="dash-player"><b>'+esc(name)+'</b><div class="dash-score">'+Number(s.points||0)+' pts</div><div class="dash-detail">Week '+current+': '+rec+'</div><div class="dash-season">Season: '+season[name]+' pts<br>Season $: <span class="money-amount '+(dollars[name]>0?'money-pos':dollars[name]<0?'money-neg':'money-zero')+'">'+(dollars[name]>0?'+':dollars[name]<0?'−':'')+'}).join('')+'</div>';
