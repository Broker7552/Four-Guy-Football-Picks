  ...
  async function loadDashboard(){
    ...
    try{
      ...
      document.getElementById('dash-grid').innerHTML='<div class="dash-grid">'+ORDER.map(name=>{
        ...
        return '<div class="dash-player"><b>'+esc(name)+'</b><div class="dash-score">'+Number(s.points||0)+' pts</div><div class="dash-detail">Week '+current+': '+rec+'</div><div class="dash-season">Season thru Week '+current+': <span style="color:#16833b">'+season[name]+' pts</span><br>Money thru Week '+moneyThrough+': <span class="money-amount '+cls+'">'+amount+'</span></div></div>';
      }).join('')+'</div>';
